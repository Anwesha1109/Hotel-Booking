...
const HotelReg = ()=>{
    ...
    return(
        // onClick={()=> setShowHotelReg(false)}- if we click any where in the form except input field it will close it
    <div onClick={()=> setShowHotelReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
        <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()}
         className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
            ...
            {/* we will add the close icon so that we can close this component when we open it */}
            <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
               ...
               {/* Address */}
                ...
                <button className='bg-indigo-500 hover:bg-indigo-600 transition-all  text-white mr-auto px-6 py-2 rounded'>
                    Regiter
                </button>
